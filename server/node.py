...
def mine_block():
    ...
    blockchain.add_transactions(sender=node_address, receiver="Miner", amount=10)
...
# 🌟 مسار API جديد لإعادة تعيين السلسلة 🌟
@app.route("/reset_chain", methods=['GET'])
def reset_chain():
    is_reset = blockchain.reset_chain()
    if is_reset:
        response = {'message': "Blockchain has been reset to Block 1."}
        return jsonify(response), 200
